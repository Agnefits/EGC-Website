...
Future<Response> postAnnouncementHandler(Request request) async {
  try {
    var payload = await request.readAsString();
    var announcementData = jsonDecode(payload);

    // Retrieve sender's information from the request
    String senderId = announcementData['senderId'];
    String senderName = announcementData['senderName']; // The name of the sender (Doctor or TA)
    String senderRole = announcementData['senderRole']; // The role of the sender (Doctor or Teaching Assistant)

    // Save the announcement with the sender information
    AnnouncementService.saveAnnouncement(
      ...
      null, // Handle file uploads if needed
    );

    return Response.ok(
      jsonEncode({'message': 'Announcement posted successfully'}),
      headers: {'Content-Type': 'application/json'},
    );
  } catch (e) {
    return Response.internalServerError(body: 'Error posting announcement: $e');
  }
}

Future<Response> getAnnouncementsHandler(Request request) async {
  try {
    var announcements = await AnnouncementService.fetchAnnouncements();

    // Assuming announcements is a list of maps with sender details
    return Response.ok(
      jsonEncode(announcements),
      headers: {'Content-Type': 'application/json'},
    );
  } catch (e) {
    return Response.internalServerError(body: 'Error fetching announcements: $e');
  }
}
...
void main() async {
  ...
  // Posting announcements from staff
router.post('/staff/announcements', postAnnouncementHandler);

// Getting announcements for students
router.get('/student/announcements', getAnnouncementsHandler);
  ...
  router.get('/staff/Course/TeachingAssistants', getCourseTeachingAssistantsHandler);
  router.get('/staff/Course/JoinTeachingAssistant',getCourseJoinTeachingAssistantsHandler);
  ...
  router.get('/staff/Course/AssignmentDetails', getCourseAssignmentDetailsHandler);
