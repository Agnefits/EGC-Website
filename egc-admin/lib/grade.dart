...
static List<Map<String, dynamic>> getGrades(int courseId) {
  final results = _db.select('''
    ...
    WHERE s.year_level = (SELECT year FROM courses WHERE id = ?) AND s.department = (SELECT department FROM courses WHERE id = ?)
...
static List<Map<String, dynamic>> fetchStudentGrades(int studentId) {
    ...
    final results = db.select(
        'SELECT c.finalExamDegree as cfinal, c.midtermDegree as cmid, c.practicalDegree as cpract, c.sectionAttendance as csec, c.lectureAttendance as clec, s.name as student, c.id , g.course_id, g.finalExamDegree , g.midtermDegree , g.practicalDegree , g.sectionAttendance , g.lectureAttendance, c.name as course from courses c left JOIN student_course_degrees g  on g.course_id = c.id left JOIN students s on g.student_id = s.id WHERE g.student_id = ?',
        [studentId]);
...
router.get('/get-grades/<courseId>', (Request request, String courseId) async {
      try {
        final grades = DatabaseHelper.getGrades(int.parse(courseId));
        return Response.ok(
          jsonEncode(grades),
          headers: {
        ...
        print("Error fetching grades: $e"); // Print the error for debugging
        return Response.internalServerError(body: 'Error: $e');
      }
    });
